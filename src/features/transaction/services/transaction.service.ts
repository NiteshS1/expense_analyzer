...
export class TransactionService implements ITransactionService {
  ...
  async getAllTransactions(
    ...
    const { transactions, totalItems } =
      await this.repository.getAllTransactions(page, limit, period, date);
  ...
  async getTransactionsByUserId(
    ...
    const { transactions, totalItems } =
      await this.repository.getTransactionsByUserId(
        id || '',
        page,
        limit,
        period,
  ...
  async getTransactionsByCategoryWithTotalAmount(
    ...
    const result = await this.repository.getTransactionsByUserId(
      id || '',
      1,
      1000,
    );
  ...
  async getTransactionsByType(
    ...
    const { transactions, totalItems } =
      await this.repository.getTransactionsByType(
        id || '',
        type,
        page,
        limit,
  ...
  async createSavingCategoryGoal(
    token: string,
    ...
    modelData.validate();
    const savingCategoryGoal =
      await this.repository.createSavingCategoryGoal(modelData);
    return savingCategoryGoal;
  }

  async getSavingCategoryGoalByUserId(
    token: string,
  ): Promise<SavingCategoryGoal[]> {
    const { valid, id } = await this.authService.validateToken(token);
    if (!valid) {
    ...
    const savingCategoryGoal =
      await this.repository.getSavingCategoryGoalByUserId(id || '');
    ...
    const expensesResult = await this.repository.getTransactionsByType(
      id || '',
      type,
      1,
      1000,
    ...
    for (const goal of savingCategoryGoal) {
      goal.expenses_amount =
        categoryMap.get(goal.category as unknown as string) ?? 0;
  ...
  async updateSavingCategoryGoal(
    id: string,
    data: UpdateSavingCategoryGoalDTO,
  ): Promise<SavingCategoryGoal | null> {
    data.validate();
    const savingCategoryGoal = await this.repository.updateSavingCategoryGoal(
      id,
      data,
    );
    if (!savingCategoryGoal) {
  ...
  async getTransactionAnalytics(
    ...
    for (const transaction of transactions) {
      if (transaction.type === TransactionType.WITHDRAWAL) {
        ...
        groupedByDay.set(
          key,
          (groupedByDay.get(key) || 0) + transaction.amount,
        );
      }
    }
